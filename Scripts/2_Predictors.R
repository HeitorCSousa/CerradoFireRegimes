...
all_lulc_files <- list.files(data_directory, pattern = "\\.tif$", full.names = TRUE)
lulc_timeseries_terra <- rast(all_lulc_files)
print(lulc_timeseries_terra)

legend_dt <- read.delim2("~/Documents/mapbiomas_lulc_10/Codigos-da-legenda-colecao-10.csv")
...
template_raster <- terra::project(template_raster, crs(lulc_timeseries_terra), threads = T, method = "bilinear")
...
results_list <- foreach(
  ...
  frac_coverage <- exact_extract(lulc_raster, grid_polygons_sf, fun = "frac",
                                 append_cols = "plot_id")
  
  # --- Convert the wide output to a long data.table ---
  setDT(frac_coverage)

  long_dt <- melt(frac_coverage, 
                  id.vars = "plot_id", 
  ...
  output_filename <- file.path(output_folder, paste0("lulc_results_", year, ".csv"))
...
lulc_wide <- dcast(all_years_long, 
                   plot_id + year ~ code_id, 
                   value.var = "fraction", 
                   fill = 0) # Fill with 0 for classes not present
...
frequency_table <- melt(frequency_counts, 
...
for (col in lulc_cols_in_data) {
  ...
  years_present_per_plot <- lulc_with_area[, .(years_present = sum(get(col) > 0)), by = plot_id]
  
  # Count how many plots had the class present every single year
  persistent_plots_count <- sum(years_present_per_plot$years_present == num_years)
  
  persistence_list[[col]] <- data.table(class_name = col, persistent_plots_count = persistent_plots_count)
}
...
non_veg_cols <- c("Photovoltaic Power Plant (beta)", "Mining", "Beach, Dune and Sand Spot", 
                  "Other non Vegetated Areas", "Hypersaline Tidal Flat", "Rocky Outcrop")
...
lulc_with_area[, non_vegetated_areas := rowSums(.SD, na.rm = TRUE), .SDcols = non_veg_cols]

# 2. Aggregate Forest Classes
lulc_with_area[, forest := rowSums(.SD, na.rm = TRUE), .SDcols = forest_cols]

# 3. Aggregate Temporary Crops
lulc_with_area[, temporary_crop := rowSums(.SD, na.rm = TRUE), .SDcols = temp_crop_cols]

# 4. Aggregate Perennial Crops
lulc_with_area[, perennial_crop := rowSums(.SD, na.rm = TRUE), .SDcols = perennial_crop_cols]

# 5. Aggregate Water Classes
lulc_with_area[, water := rowSums(.SD, na.rm = TRUE), .SDcols = water_cols]

# 6. Aggregate wet vegetation classes
lulc_with_area[, wet_herbaceous := rowSums(.SD, na.rm = TRUE), .SDcols = wet_herbaceous_cols]

# --- Now, remove all the original columns at once ---
cols_to_remove <- c(non_veg_cols, forest_cols, temp_crop_cols, perennial_crop_cols, water_cols, wet_herbaceous_cols)
...
native_classes <- c("forest", "savanna", 
                    "wet_herbaceous", "grassland", "water")

anthropic_classes <- c("pasture", "temporary_crop", "perennial_crop", "soybean", "sugar_cane","forest_plantation", 
                       "mosaic_uses", "non_vegetated", "urban")
...
lulc_with_area[, native_area := rowSums(.SD, na.rm = TRUE), .SDcols = native_classes]
lulc_with_area[, anthropic_area := rowSums(.SD, na.rm = TRUE), .SDcols = anthropic_classes]
...
saveRDS(lulc_with_area, "~/Documents/mapbiomas_lulc_10/yearly_lulc_results/lulc_predictors.rds")
...
all_ndvi_files <- list.files(data_directory, pattern = "\\.tif$", full.names = TRUE)
...
ndvi_timeseries_terra <- resample(ndvi_timeseries_terra, grid_9km, method = "bilinear", threads = T)
print(ndvi_timeseries_terra)
...
ndvi_timeseries_df <- as.data.frame(ndvi_timeseries_terra, cells = TRUE, xy = TRUE, wide = FALSE)

# Rename columns
names(ndvi_timeseries_df) <- c("plot_id", "longitude", "latitude", "year", "NDVI")
...
layers_2020 <- names(pop_grid_cerrado)[str_detect(names(pop_grid_cerrado), "_2020")]
...
layers_2030 <- names(pop_grid_cerrado)[str_detect(names(pop_grid_cerrado), "_2030")]
# Calculate the mean of these layers
...
pop_timeseries_terra <- resample(pop_timeseries_terra, grid_9km, method = "bilinear", threads = T)
...
# Extract data to data.frame
pop_timeseries_df <- as.data.frame(pop_timeseries_terra, cells = TRUE, xy = TRUE, wide = FALSE)
...
dewpoint_temp <- terra::project(dewpoint_temp, metric_crs, threads = T, method = "bilinear")
temp <- terra::project(temp, metric_crs, threads = T, method = "bilinear")
pot_evap <- terra::project(pot_evap, metric_crs, threads = T, method = "bilinear")
precip <- terra::project(precip, metric_crs, threads = T, method = "bilinear")
skin_temp <- terra::project(skin_temp, metric_crs, threads = T, method = "bilinear")
solrad <- terra::project(solrad, metric_crs, threads = T, method = "bilinear")
total_evap <- terra::project(total_evap, metric_crs, threads = T, method = "bilinear")
wind <- terra::project(wind, metric_crs, threads = T, method = "bilinear")
water_soil <- terra::project(water_soil, metric_crs, threads = T, method = "bilinear")
water_deficit <- terra::project(water_deficit, metric_crs, threads = T, method = "bilinear")
...
for (var_name in names(raster_list)) {
  ...
  var_dt <- as.data.frame(current_raster, cells = TRUE, time = TRUE, xy = TRUE, wide = FALSE)
  setDT(var_dt)
...
saveRDS(all_variable_summaries, "~/Documents/ERA5/climate_variable_results/list_climate_predictors.rds")
...
final_climate_dt_complete <- Reduce(
  function(x, y) merge(x, y, by = c("cell", "x", "y", "year", "month"), all = FALSE),
  prepared_list
)
...
# Save results
saveRDS(final_climate_dt_complete, "~/Documents/ERA5/climate_variable_results/climate_predictors.rds")
...
Cerrado <- Cerrado %>% dplyr::select(geometry)
Cerrado <- sf::st_transform(Cerrado, metric_crs)
elevation_dem <- get_elev_raster(locations = Cerrado, 
                                 z = 9, 
                                 prj = "EPSG:5880",
                                 clip = "locations")
...
terrain_vars <- terrain(rast(elevation_dem), 
