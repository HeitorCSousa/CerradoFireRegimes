...
#-----------------------------------------------------------------------
# STEP 2: SET UP ENVIRONMENT AND LIST FILES
#-----------------------------------------------------------------------
...
output_directory <- file.path("~/Documents/mapbiomas_fogo_4", "MERGED_ANNUAL_RASTERS")
...
#-----------------------------------------------------------------------
# STEP 3: BATCH PARALLEL PROCESSING (SAVE TO DISK)
#-----------------------------------------------------------------------
...
years_to_process <- unique_years[!sapply(unique_years, function(year) {
  file.exists(file.path(output_directory, paste0("merged_fire_", year, ".tif")))
})]
...
if (length(years_to_process) == 0) {
  ...
  # Create the batches
  batches <- split(years_to_process, ceiling(seq_along(years_to_process) / batch_size))
  
  # Set up the parallel cluster
  num_cores <- min(detectCores() - 1, batch_size)
  registerDoParallel(cores = num_cores)
  cat("Processing in batches of up to", batch_size, "using", num_cores, "cores.\n")
  
  # Loop through each batch
  for (i in seq_along(batches)) {
    current_batch <- batches[[i]]
    cat("--- Starting Batch", i, "of", length(batches), "--- Years:", paste(current_batch, collapse=", "), "---\n")
    ...
    foreach(
      ...
      output_filename <- file.path(output_directory, paste0("merged_fire_", current_year, ".tif"))
...
#-----------------------------------------------------------------------
# STEP 4: STACK RESULTS FROM DISK
#-----------------------------------------------------------------------
# List all the generated files and stack them
# This part remains the same
all_output_files <- list.files(output_directory, pattern = "\\.tif$", full.names = TRUE)
...
# Carregar a geometria do Cerrado
Cerrado <- readRDS("Cerrado.rds")
print(Cerrado)

# Selecionar apenas a geometria do objeto "Cerrado"
...
# Part 1: Generate Binary Rasters
...
# This is the CRITICAL step to fix the warning and ensure correct metric values.
# This may take some time.
cat("Reprojecting time series to metric CRS:", metric_crs, "...\n")
fire_timeseries_metric <- terra::project(fire_timeseries_terra, metric_crs, threads = T, method = "near")

writeRaster(fire_timeseries_metric, paste0(file.path("~/Documents/mapbiomas_fogo_4", "MERGED_ANNUAL_RASTERS", "fire_timeseries/"), "fire_timeseries_metric.tif"),
            datatype = "INT1U", overwrite = T)

fire_timeseries_metric <- rast(paste0(file.path("~/Documents/mapbiomas_fogo_4", "MERGED_ANNUAL_RASTERS", "fire_timeseries/"), "fire_timeseries_metric.tif"))
...
sample_extent <- ext(center_x - 50000, center_x + 50000, center_y - 50000, center_y + 50000)
...
all_tasks <- all_tasks %>%
  mutate(
    binary_filename = file.path(binary_output_dir, paste0("binary_fire_", year, "_", sprintf("%02d", month), ".tif"))
  )
...
if (nrow(tasks_to_process) == 0) {
  ...
  batches <- split(tasks_to_process, ceiling(seq_len(nrow(tasks_to_process)) / batch_size))
...
binary_output_dir <- file.path(data_directory, "BINARY_MONTHLY_RASTERS")
binary_files <- list.files(binary_output_dir, pattern = "\\.tif$", full.names = TRUE)
...
template_raster <- terra::project(template_raster, metric_crs, threads = T, method = "bilinear")
...
all_files <- list.files(binary_input_dir, pattern = "\\.tif$", full.names = TRUE)

# Create a list of files that still need to be processed
files_to_process <- Filter(function(file_path) {
  rds_filename <- file.path(rds_output_dir, paste0(tools::file_path_sans_ext(basename(file_path)), ".rds"))
...
if (length(files_to_process) == 0) {
  ...
  # --- PARALLEL BATCH PROCESSING ---
  batch_size <- 20
  batches <- split(files_to_process, ceiling(seq_along(files_to_process) / batch_size))
  ...
  for (i in seq_along(batches)) {
    ...
    foreach(file_path = current_batch, .packages = c("terra", "landscapemetrics")) %dopar% {
      
      filename <- basename(file_path)
      rds_filename <- file.path(rds_output_dir, paste0(tools::file_path_sans_ext(filename), ".rds"))
...
rds_output_dir <- file.path("~/Documents/mapbiomas_fogo_4/BINARY_MONTHLY_RASTERS/METRICS_RDS_FILES")
...
filtered_metrics_dt <- all_metrics_df[selected_metrics, 
...
metrics_wide_sparse <- dcast(filtered_metrics_dt, 
                             plot_id + class + year + month ~ function_name, 
                             value.var = "value",
                             fun.aggregate = mean)
saveRDS(metrics_wide_sparse, "~/Documents/mapbiomas_fogo_4/BINARY_MONTHLY_RASTERS/METRICS_RDS_FILES/FILTERED/metrics_wide_sparse.rds")

metrics_wide_sparse <- readRDS("~/Documents/mapbiomas_fogo_4/BINARY_MONTHLY_RASTERS/METRICS_RDS_FILES/FILTERED/metrics_wide_sparse.rds")
...
percentage_lookup_original <- unique(filtered_metrics_dt[, .(plot_id, percentage_inside)])
...
percentage_lookup <- filtered_metrics_dt[percentage_inside > 0, 
...
class_data_to_merge <- class_data[, c(key_cols, class_metric_cols), with = FALSE]

# Identify which columns are ONLY landscape-level metrics
landscape_metric_cols <- names(landscape_data)[str_starts(names(landscape_data), "lsm_l_")]

landscape_data_to_merge <- landscape_data[, c(key_cols, landscape_metric_cols), with = FALSE]
...
master_grid <- CJ(plot_id = all_ids,
                  year = all_years,
                  month = all_months)
...
missing_combinations <- master_grid[!actual_data_keys,
...
if (nrow(missing_combinations) == 0) {
  ...
  print(paste("Found", nrow(missing_combinations), "missing plot/date combinations."))
...
ggplot(final_data[final_data$year == 2023 & final_data$month == 9,],
       aes(x = longitude, y = latitude, color = lsm_c_lpi)) +
  geom_point(shape = 15, size = 1.5) +
  scale_color_viridis_c(name = "LPI value") +
  labs(title = "Spatial Distribution of Largest Patch Index (LPI) - September 2023",
       x = "Longitude (m)", y = "Latitude (m)") +
  theme_minimal() +
  coord_equal()

summary(final_data)
saveRDS(final_data, "~/Documents/mapbiomas_fogo_4/BINARY_MONTHLY_RASTERS/METRICS_RDS_FILES/FILTERED/final_metrics_data.rds")
...
na_percentages <- final_data[, lapply(.SD, function(x) sum(is.na(x) | is.nan(x)) / .N * 100),
...
vars_to_remove <- c(names(na_vector[na_vector == 100 | na_vector > 5]), "lsm_l_pr", "lsm_l_prd")

print("Removing the following unreliable variables:")
print(vars_to_remove)

# Create a new, clean vector of predictor names
final_metric_vars <- c("plot_id", "year", "month", "latitude", "longitude", setdiff(selected_metrics$function_name, vars_to_remove))
...
# Correlation plot
correlation_matrix <- cor(final_clean_data[, 6:12], use = "pairwise.complete.obs", method = "spearman")
...
corrplot(correlation_matrix, 
         ...
         tl.col = "black", tl.srt = 45, # Tweak text labels
         diag = FALSE) # Hides the diagonal
dev.off()

saveRDS(final_clean_data, "~/Documents/mapbiomas_fogo_4/BINARY_MONTHLY_RASTERS/METRICS_RDS_FILES/FILTERED/final_clean_data.rds")



# Run the PCA
pca_fire_metrics <- prcomp(final_clean_data[, 6:12], center = TRUE, scale. = TRUE)
...
autoplot(pca_fire_metrics ,
  ...
  labs(title = "PCA of Fire Landscape Metrics by Decade",
...
ggplot(decadal_means_dt,
       aes(x = longitude, y = latitude, fill = lsm_c_cai_sd)) +
  geom_raster() +
  facet_grid(decade ~ month) +
  scale_fill_viridis_c(name = "SD CAI", na.value = "transparent", transform = "log1p", option = "inferno") +
  coord_equal() +
  labs(title = "SD CAI by decade and month",
       x = "Longitude (m)",
       y = "Latitude (m)") +
  theme_minimal() +
...
ggplot(decadal_means_dt,
  ...
  geom_raster() +
  facet_grid(decade ~ month) +
  scale_fill_viridis_c(name = "Mean circle", na.value = "transparent", transform = "log1p", option = "inferno") +
  coord_equal() +
  labs(title = "Mean of related circumscribing circle",
       x = "Longitude (m)",
       y = "Latitude (m)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 6),
...
ggplot(decadal_means_dt,
  ...
  geom_raster() +
  facet_grid(decade ~ month) +
  scale_fill_viridis_c(name = "Mean core area", na.value = "transparent", transform = "log1p", option = "inferno") +
  coord_equal() +
  labs(title = "Mean core area",
       x = "Longitude (m)",
       y = "Latitude (m)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 6),
...
ggplot(decadal_means_dt,
  ...
  geom_raster() +
  facet_grid(decade ~ month) +
  scale_fill_viridis_c(name = "SD number of disjuct core areas", na.value = "transparent", transform = "log1p", option = "inferno") +
  coord_equal() +
  labs(title = "SD number of disjuct core areas",
       x = "Longitude (m)",
       y = "Latitude (m)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 6),
...
ggplot(decadal_means_dt,
  ...
  geom_raster() +
  facet_grid(decade ~ month) +
  scale_fill_viridis_c(name = "SD ENN", na.value = "transparent", transform = "log1p", option = "inferno") +
  coord_equal() +
  labs(title = "SD ENN",
       x = "Longitude (m)",
       y = "Latitude (m)") +
...
ggplot(decadal_means_dt,
       aes(x = longitude, y = latitude, fill = lsm_c_pd)) +
  geom_raster() +
  facet_grid(decade ~ month) +
  scale_fill_viridis_c(name = "Density of fire scars", na.value = "transparent", transform = "log1p", option = "inferno") +
  coord_equal() +
  labs(title = "Density of fire scars",
       x = "Longitude (m)",
       y = "Latitude (m)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 6),
...
ggplot(decadal_means_dt,
       aes(x = longitude, y = latitude, fill = lsm_c_split)) +
  geom_raster() +
  facet_grid(decade ~ month) +
  scale_fill_viridis_c(name = "Splitting index", na.value = "transparent", transform = "log1p", option = "inferno") +
  coord_equal() +
  labs(title = "Splitting index",
       x = "Longitude (m)",
       y = "Latitude (m)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 6),
...
complete_fire_data <- fire_occurrence_dt[master_grid, 
                                         on = c("plot_id", "year", "month")]
...
saveRDS(complete_fire_data, "~/Documents/mapbiomas_fogo_4/BINARY_MONTHLY_RASTERS/METRICS_RDS_FILES/FILTERED/final_fire_occ_data.rds")
...
ggplot(decadal_fire_occ_dt,
       aes(x = longitude, y = latitude, fill = fire_occurrence)) +
  geom_raster() +
  facet_grid(decade ~ month) +
  scale_fill_viridis_c(name = "Fire occurrence", na.value = "transparent", option = "inferno") +
  coord_equal() +
  labs(title = "Fire occurrence by decade and month",
       x = "Longitude (m)",
       y = "Latitude (m)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 6),
